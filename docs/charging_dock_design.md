...
> 版本：v1.2（設計定案，未施工）
> 最後更新：2026-08-11
> 🔵 **2026-08-11 充電器定案**：充電器選 **Mean Well HLG-150H-30A**（NT$940），**取代原案 HLG-320H-30A**（NT$2,583–2,900）。
> 理由：功率夠用（28.0V × 5A ＝ 140W ≤ 額定 150W）＋ 省錢（−NT$1,643～1,960）。
> **充電電流由 10.7A 降為 5A**，全文以 10.7A 為基準的數字皆已改算（保護階梯裕度、分流器使用率、充飽時間、成本）。
> 影響 **§0.2 裁決 7、§1、§2.1 方塊圖、§3.2、§3.5、§3.7、§7、§8.3、§9.3、§11.1 D1、§11.4、§14 R-5、§15 決策 2/16、§16**。
> **保護件額定（F-D1 15A、刷板 20A、K1 30A）與所有軟體門檻一律不變**，理由見 §3.2「換充電器為什麼不必重選保護件」。
> 🔴 **2026-08-11 AC 側定案**：dock 插座為 **110V 單相**（先前全文以 220V 為前提）。
> AC 保護由「2A 慢熔」改為 **RCBO 30mA + C 曲線 C6**（滿載 AC 依 SPEC：HLG-150H **1.7A @115VAC**，2A 會熔斷）。
> 影響 **§0.2 裁決 6、§2.1 方塊圖、§3.7、§8.3、§11.1 D20、§11.4、§16**。**DC 側完全不受影響。**
> 🔄 **2026-08-11 TP-PROBE 裁決**：dock 原理圖的 `J3`（TP-PROBE_RSV 試探排針）**刻意刪除**，
> **v1 連走線與排針都不做**（不是誤刪）。理由：v1 簡化，需要量 K1 下游電壓時用萬用表直接接端子台即可。
> 文件採**封存不刪文**處置，WD-P1/WD-P2 與 BOM 排針列保留並標記 v2+ 預留。
> 影響 **§0.2 裁決 1、§2.1 方塊圖、§2.2、§4.4、§8.3、§11.2 D27、§11.3、§14 R-1／R-12**。
...
| # | 裁決 | 本文件的落實位置 |
|---|---|---|
| 1 | 上電邏輯 = 微動開關 ×2（左右各一，同時觸發）→ ESP32 → 主繼電器閉合。**不做電壓試探**；擴充點只保留在文件上，🔄 **v1 連走線與排針都不施作**（2026-08-11 裁決，圖面 `J3` 已刪） | §3.4、§6、**§4.4**、§11 #P1–P4 |
...
| 7 | 🔵 **充電器 ＝ Mean Well HLG-150H-30A**（2026-08-11 拍板），**取代原案 HLG-320H-30A**。<br/>理由：**功率夠用**（28.0V × 5A ＝ 140W ≤ 額定 150W，且 28.0V 落在 CC 區間 15–30V 內、CV 可調 27–33V 涵蓋 28.0V）＋ **省錢**（NT$940 vs NT$2,583–2,900，省 NT$1,643–1,960）。<br/>代價已知並接受：**CC 電流 10.7A → 5A**，充飽時間 3.6h → **5.7h**（dock 為常時停靠浮充架構，不痛）；充電倍率由 0.28C 降為 **0.167C**，低於 §1 原訂 0.2–0.3C 目標 | §1、§3.2、§3.5、§7、§11.1 D1、§11.4、§15 決策 16 |
...
| 充電電流目標（原訂） | 0.2–0.3C ≈ **6–9A** | 本工單 |
| 🔵 **充電器實際 CC 電流（定案）** | **5.0A**（Mean Well **HLG-150H-30A** 額定；CC 可調 3–5A，🔴 **VR 設滿 5A**） | SPEC `HLG-150H-SPEC.PDF` OUTPUT 段 |
| 🔵 實際充電倍率 | 5.0A / 30Ah ＝ **0.167C**（⚠️ **低於原訂 0.2–0.3C**） | 5 ÷ 30 |
| 🔵 充飽時間 | **≈ 5.7 小時**（原案 320H @10.7A 為 3.6h） | 模型見下 |

> 🔵 **0.167C 低於原訂目標，這是拍板時已知並接受的代價**（§0.2 裁決 7）。
> dock 是**常時停靠浮充**架構——機器人回站就掛著充，不趕在固定時窗內充完，
> 因此充電倍率不是硬需求。**若日後使用強度提高、5.7h 不夠用，這是第一個會撞到的限制**（列為 §14 R-13）。
>
> **充飽時間模型**（全文統一，可複驗）：起點 21.7V（≈5% SoC，＝ `power_design.md` §2 的停機門檻）
> → 終點 CV 28.0V（4.0V/cell，≈88% SoC），需補 **≈25Ah**；其中 CC 段補 80%（20Ah）、CV 尾段固定約 1.7h ⇒
> ```
> t ≈ 20Ah ÷ I_cc + 1.7h
> ```
> **I_cc = 5.0A ⇒ t ≈ 20 ÷ 5 + 1.7 ＝ 4.0 + 1.7 ＝ 5.7h**（CC 段 4.0h、CV 尾段 1.7h）。
> 對照原案 I_cc = 10.7A ⇒ 20 ÷ 10.7 + 1.7 ＝ 1.87 + 1.7 ≈ **3.6h**。
>
> **輸出功率驗算**：28.0V × 5.0A ＝ **140W ≤ 150W 額定** ✅（餘裕 7%）。
> 28.0V 亦落在 SPEC 的 CC 區間 **15–30V** 內 ⇒ 5A 恆流成立，不會提前掉出 CC 段。
...
flowchart LR
    subgraph DOCK["充電 Dock（獨立基礎設施）"]
        ...
        CHG["充電器 HLG-150H-30A<br/>28.0V / 5A<br/>CC+CV"]
        ...
        SWL["SW-L 微動"]
        SWR["SW-R 微動"]
        TP["TP-PROBE<br/>電壓試探擴充點<br/>🔄 v1 完全不施作<br/>（2026-08-11 裁決，v2+ 預留）"]
    end
...
| 層級 | 動作點 | 反應時間 | 由誰執行 |
|---|---|---|---|
| 正常充電 | 🔵 **3–5A**（HLG-150H-30A CC 可調範圍，**VR 設滿 5A**） | — | 充電器 CC 限流 |
...
| 檢查 | 數值（🔵 以定案的 HLG-150H-30A ＝ **5A** 重算） | 結果 |
|---|---|---|
| 軟體切斷 12A vs 充電器上限 5A | 12 ÷ 5 ＝ **2.40 倍**（原案 10.7A 時為 1.12 倍） | 正常充電不誤跳 ✅（裕度較原案更寬） |
| F-D1 15A vs 軟體切斷 12A | 15 ÷ 12 ＝ **1.25 倍** | 軟體先動作，保險絲是備援 ✅（**與充電器無關，不變**） |
| F-D1 15A vs 充電器 5A | 15 ÷ 5 ＝ **3.00 倍**（原案為 1.40 倍） | CC 滿載連續運轉不熔斷 ✅ |
| 刷板 20A vs F-D1 15A | 20 ÷ 15 ＝ **1.33 倍** | **保險絲低於刷板額定，層級正確** ✅（**與充電器無關，不變**） |
| K1 30A/30VDC vs 5A | 5 ÷ 30 ＝ 負載率 **16.7%**（原案為 36%） | 降額充足，抑制接點熔接 ✅ |

> **刷板 20A 仍是最低的元件額定天花板**（K1 改模組後升到 30A）。保險絲 15A < 刷板 20A 的層級關係不變。
> ⚠️ 🔵 **但「20A 刷板 + 15A 保險絲是唯一可行組合」這個更強的宣稱，已隨充電器定案而失效**——
> 它依賴的是「10A 保險絲 < 10.7A 會誤熔斷」，在 5A 下不成立。詳見下方第三段與 §14 R-14。

> 🔵 **換充電器為什麼不必重選保護件（本節的核心判準）**
>
> 保護階梯的成立條件是**「每一層低於下一層」**，而這條不等式鏈裡只有**最底層**（正常充電電流）
> 跟充電器有關，其餘四層（軟體警告 / 軟體切斷 / F-D1 / 元件額定）**彼此的相對關係與充電器無關**。
> 320H → 150H 只是把最底層從 10.7A 拉低到 5A ⇒ **所有裕度倍數全部變大，沒有任何一格由 ✅ 變 ❌**。
> 因此 **F-D1 15A、刷板 20A、K1 30A 三個保護件額定一律不變**——重選只增加採購成本，不增加安全。
>
> 🔴 **軟體門檻（11.5 / 12.0 / 14.0A）同樣不變，但要誠實說明它現在保護什麼：**
> 5A 的 CC 環路正常時，**下游短路也只會被箝在 5A**，軟體過流門檻**實務上不會被觸發**——
> 這不是缺陷，因為軟體過流保護的標的**從來就不是「正常 CC 電流太大」**，而是
> **① 充電器 CC 環路失效**（輸出不再受限，電流可衝到 12A 以上）與 **② 車端防倒灌失效導致電池回灌**
> （電池短路電流可達數十 A，§9.1）。這兩種情境下 12A/14A 仍會在 F-D1 15A 之前先動作，
> **階梯的功能完整保留**。把門檻收緊到接近 5A 反而會被入座湧浪與量測誤差誤觸發，得不償失。

> 🔵 **刷板仍選 20A 檔（決定不變，但原本的夾擠理由已不再成立——誠實記錄）**
> 原本的論證是：「15A 刷板 ⇒ 保險絲得降到 10A ⇒ 10A < 充電器 CC 上限 10.7A ⇒ 滿載誤熔斷」。
> 換成 150H 之後 **CC 上限只有 5A，10A 保險絲不會誤熔斷**，這條夾擠**在數學上已經不再夾死**，
> 15A 刷板 + 10A 保險絲在 5A 充電下是可行的。
> **仍維持 20A 的理由改為**：① 刷板是**接觸件**，額定裕度直接換成接觸電阻與壽命，20A 檔對 5A 是 4× 降額；
> ② F-D1 15A 與車上 F1 同族、備品共用（§11.1 D4），改 10A 就破壞共用；
> ③ 刷板尚未下單（§11.1 D2 數量 0，等詢價），**改選 15A 檔省不了多少，卻要動整條 BOM 與 §15 決策 2**。
> ⚠️ **若賣家 15A 檔明顯便宜且使用者願意接受**，改選是合法的——但那是新的採購裁決，**不在本次變更範圍**（見 §14 R-14）。
...
| 項目 | 數值 | 本設計用量 | 餘裕 |
|---|---|---|---|
| **接點額定（DC）** | **30A / 30VDC**（本體印字直接可讀，見 A-3） | 🔵 **5A @ 28.0V**（HLG-150H-30A 定案，§0.2 裁決 7） | 🔵 **16.7% 負載率** ✅（原案 10.7A 時為 36%） |
...
> 14 AWG 安全載流 15–20A（`power_design.md` §5.1），對 🔵 **5A**（定案）有 **3–4× 餘裕**；
> 🔵 **線徑不因換充電器而縮小**——12 AWG 是依 F-D1 15A 的保護上限選的，不是依充電電流選的
> （導線必須撐得住保險絲熔斷前的電流，否則保護鏈的順序會反過來）。
...
| 檢查 | 數值（🔵 以定案的 **5A** 重算） | 結果 |
|---|---|---|
| 🔵 5A 時分流器壓降 | 5 × 2.5mΩ = **12.5mV**（原案 10.7A 時為 26.8mV） | 滿刻度 81.92mV 的 **15.3%** ✅ |
| 🔵 分流器功耗 | 5² × 0.0025 = **0.0625W ＝ 62.5mW**（原案為 0.29W） | FL-2 為 30A 額定，遠低於其耗散能力 ✅ |
| 解析度 | 1 mA | 對 0.3A 充飽門檻 ＝ 300 階，綽綽有餘 ✅ |
| 🔵 **過流門檻仍在量程內** | 14.0A × 2.5mΩ ＝ 35mV | 滿刻度的 43%，`I_oc_fast` 讀得到 ✅（門檻不因換充電器而失效） |

> 🔵 **滿刻度使用率降到 15% 是可接受的，不需要換分流器。**
> INA226 是 16-bit ΔΣ，Current_LSB 固定 1mA ⇒ **解析度與滿刻度使用率無關**：
> 5A 讀值仍有 5000 階，0.3A 充飽門檻仍有 300 階。
> 換小阻值分流器只會讓 `CAL` 與韌體常數全部重算（§3.5 開頭「韌體常數不需重算」的前提就沒了），
> 且 FL-2 30A 與車上 `power_design.md` W4 同款、備品共用——**維持原選型**。

**🔴 高側接法的三個必要條件：**

1. **`R100` 必須拆除**。INA226 模組板自帶的 0.1Ω 取樣電阻只能量到 0.8A，
   接上 **5A**（＝ 6.3 倍）一樣會直接燒毀——**充電電流降到 5A 完全不改變這條紀律**。
   與 `power_design.md` §15.2、`ina226.h:19` 是同一條紀律
...
| AC 安全（沿用 `power_design.md` §15.7，**必須項非建議項**） | ① 裝入封閉外殼，AC 端子不得可觸及<br/>② 🔴 **AC 側裝 RCBO 30mA + C 曲線 C6**（漏電＋過流二合一，**取代**原本的機內 2A 慢熔）<br/>③ 金屬外殼接 FG<br/>④ 若仍要保留機內慢熔作為備案，額定寫 **T5A@110V**；🔴 **2A 只適用 220V，110V 下會誤熔斷**；🔵 **定案的 HLG-150H 滿載僅 1.7A，T5A 有 2.9× 裕度**（T4A 對 150H 也夠，但**維持 T5A 不改**，理由見下表註） | dock 放在地面、有人會經過；漏電保護在地面潮濕環境是必須項，不是建議項 |

> **🔴 為什麼從「2A 慢熔」改成「RCBO 30mA + C6」**（本節數字全部以 110V 為前提）
>
> 🔵 **2026-08-11 更新**：充電器已定案 **HLG-150H-30A**（§0.2 裁決 7）。下表**保留 320H 對照值供追溯**，
> 但**所有選型結論一律以 150H 為準**；320H 對照僅用來說明「為什麼 T5A / C6 這個保守選擇在兩款下都站得住」。
>
> | 項目 | 值 | 說明 |
> |---|---|---|
> | 充電器滿載 AC 輸入電流（官方 SPEC 實值） | 🔵 **HLG-150H：1.7A（定案）**<br/><sub>（對照：HLG-320H 3.5A，已廢案）</sub><br/><sub>（AC CURRENT Typ. @115VAC）</sub> | SPEC 的 AC CURRENT 是**系列共用 typ 值**（同系列各輸出電壓版本共列一個數字），**110V 下略高**：1.7A → **~1.8A**（依 115/110 反比換算：1.7 × 115/110 ＝ 1.78A）。<br/>同一顆在 230V 下不到一半（150H 0.75A）⇒ **原本的 2A 慢熔是以 220V 算出來的**；即使定案的 150H 在 110V 只吃 ~1.8A，2A 慢熔的裕度也只有 **1.1×**，同樣站不住 ⇒ 換 RCBO 的結論**不因換充電器而動搖**。<br/>（先前文中的「~3A @110V」是估值，已以官方 SPEC 實值取代） |
> | 慢熔備案額定 | **T5A**（維持不變） | 🔵 裕度（定案 150H）：對 SPEC 1.7A ＝ **2.9×**；以 110V 換算值 1.8A 計 ＝ **2.8×**。<br/>🔵 **為什麼 150H 定案後仍不降到 T4A**：T4A 對 150H 確實也夠（2.4×／2.2×），但 T5A 是**已定案的備案料件**（§11.1 D20），降額後 3.75A 對 1.8A 仍在保護有意義的範圍內，**換料只增加採購動作、不增加保護**。<br/><sub>（對照：對已廢案的 320H 3.5A，T4A 只有 1.14×、25% 降額後只能連續帶 3.0A ＜ 3.5A ⇒ 當時 T4A 是**不可用**的。這條理由隨 320H 廢案而失效，但結論相同。）</sub> |
> | RCBO 漏電動作值 | **30mA** | 人身保護等級；dock 在地面、金屬外殼接 FG，漏電才是這個場景的主風險。**與充電器型號無關** |
> | RCBO 曲線與額定 | **C 曲線 / C6**（維持不變） | 🔵 過流裕度（定案 150H）：對 SPEC 1.7A ＝ 6 ÷ 1.7 ＝ **3.5×**；以 110V 換算值 1.8A 計 ＝ **3.3×**。<br/><sub>（對照：已廢案的 320H 為 1.7×／1.67×。）</sub><br/>🔴 **不可選 C2**：C2 的磁脫扣區間（5–10×In ＝ 10–20A）低於充電器冷開機湧浪，會誤跳；C6 的磁脫扣區間是 **30–60A**（10–20×In），湧浪峰值雖落在此區間內仍**不會**誤跳，理由見下兩列。<br/>⚠️ **150H 的湧浪較小（~31A@110V）不代表可以降到 C2**——31A 仍高於 C2 的 20A 上限 ⇒ **C2 照樣會跳**，禁用結論不變 |
> | 冷開機湧浪（官方 SPEC 實值） | 🔵 **HLG-150H：65A，twidth ＝ 425µs（定案）**<br/><sub>（對照：HLG-320H 70A / 1010µs，已廢案）</sub><br/><sub>（COLD START @230VAC；twidth 量測於 50% Ipeak；Per NEMA 410）</sub> | 湧浪峰值 ≈ 線電壓峰值 ÷ 輸入級等效阻抗 ⇒ 與電壓近似成正比，**110V 下約減半：~31A（150H）**。<br/>🔵 定案後脈寬由最壞的 1010µs 降為 **425µs**，對「脈寬 ≪ 磁脫扣動作時間」的論證**更有利**（見下列）。<br/>（先前文中的「~75A@230V」是估值，已以官方 SPEC 實值取代） |
> | 🔴 **為什麼峰值落在磁脫扣區間也不會跳** | **脈寬 0.4–1.0 ms ≪ 磁脫扣機構動作時間** | 磁脫扣是**電磁機械機構**：線圈吸動銜鐵＋推動解扣連桿需要**數 ms 等級的持續電流**才能完成行程；HLG 的湧浪是單發脈衝，半高寬只有 425µs／1010µs，在銜鐵走完行程前電流早已衰減，機構回落 ⇒ **不脫扣**。同理其 I²t 也遠低於脫扣門檻。<br/>🔵 **旁證（改用定案 150H 自己的 SPEC）**：`HLG-150H-SPEC.PDF` 的 `MAX. No. of PSUs on 16A CIRCUIT BREAKER` 一列寫 **「4 units (type B) / 7 units (type C) at 230VAC」**——**同一顆 16A C 型斷路器可以掛 7 台 HLG-150H**。若 65A 的湧浪峰值就會讓 C 型脫扣，一台都掛不了，遑論 7 台。<br/><sub>（原文引用的是已廢案 320H 的「可掛 2 台」，同樣成立但不再是本設計的本尊數據。）</sub> |
>
> **AC 電流／湧浪資料來源**：Mean Well 官方 SPEC PDF —— 🔵 **定案依據為 `meanwell.com/Upload/PDF/HLG-150H/HLG-150H-SPEC.PDF`**
> （INPUT 段 `AC CURRENT (Typ.)`、`INRUSH CURRENT (Typ.)`、`MAX. No. of PSUs on 16A CIRCUIT BREAKER` 三列）；
> 對照值出自 `.../HLG-320H/HLG-320H-SPEC.PDF`（已廢案，僅供追溯）。
> ⚠️ 若現場實測仍在冷開機瞬間跳脫，退路是升到 **C10**（磁脫扣 50–100A）；
> 代價是過流保護變鬆——C10 對定案的 150H 滿載 ＝ **5.9×In（1.7A）**，等於 AC 側幾乎只剩漏電保護在把關，
> 因此**先用 C6，跳了再升**，不要一開始就選 C10。
> 🔵 定案 150H 的湧浪僅 ~31A@110V（脈寬 425µs），落在 C6 磁脫扣區間 30–60A 的**最下緣**，
> 誤跳機率比原案 320H（~33A / 1010µs）更低 ⇒ **升 C10 的機率下降，但仍列為退路**。
>
> RCBO 一顆同時提供漏電與過流保護，**功能上涵蓋原本的機內慢熔**，所以是取代而非疊加。
> 保留慢熔備案的唯一理由是「現場只有普通插座、上游沒有可加裝 RCBO 的配電位置」——
> 走這條路時額定務必用 **T5A**，不要照抄舊文件的 2A（對定案 150H 在 110V 的 ~1.8A 僅 1.1× 裕度，照樣會誤熔斷）。
...
### 4.4 電壓試探擴充點（🔴 **預留，v1 完全不施作**）

> 🔄 **2026-08-11 裁決（使用者拍板）**：dock 原理圖上的 `J3`（TP-PROBE_RSV，V_BRUSH/GND 試探排針）
> **刻意刪除，v1 連排針與走線都不做**（不是誤刪）。理由：**v1 簡化**——試探點在 v1 沒有任何韌體會讀，
> 需要量 K1 下游電壓時直接用萬用表接端子台即可，不需要為此多拉兩條線與兩支排針。
> 下表 **WD-P1/WD-P2 兩列封存保留、不刪文**（慣例同 `power_design.md` 對廢棄方案的處置），
> 降級為 **v2+ 預留**：日後要裝料時，這兩條線與排針要**一併補做**，不能假設板上已經有。

| 線號 | 從 | 到 | 狀態 |
|---|---|---|---|
| ~~WD-P1~~ | ⊕ 端子台第三位（**K1 下游**） | ~~排針 **TP-PROBE-HI**~~ | 🔄 **v1 不施作**（2026-08-11 裁決：圖面 `J3` 已刪，連排針都不做）。<br/>⬜ **降級為 v2+ 預留**：裝料時需一併補拉此線並裝排針 |
| ~~WD-P2~~ | ⊖ 端子台 | ~~排針 **TP-PROBE-LO**~~ | 🔄 **v1 不施作**：同上 |
...
用來偵測**接點熔接**（§8.3）——v1 完全不施作時這個自檢有覆蓋盲區，見 §8.3。
🔴 **v2 施作範圍已因本次裁決放大**：不只是「把 P1–P4 的料補上」，而是
**WD-P1/WD-P2 走線 + 兩支排針 + P1–P4 全部從零做起**（工時增加，但 v1 省下的施工也是真的）。
...
| 為何能區分 | — | **變化率**：拖走是 100ms 內從 🔵 **5A** 掉到 0；充飽是 CV 階段數分鐘的漸進衰減。兩者的 di/dt 差了三個數量級（🔵 5A/0.1s ＝ 50A/s vs CV 尾段約 5A/1.7h ≈ 8×10⁻⁴ A/s，差 **4~5 個數量級**，換 150H 後區分度反而更好） | — |

**主要路徑是微動開關**，因為它最快（30ms），而快是有意義的：
斷電必須搶在刷塊機械分離之前完成，否則就是 🔵 **5A** 的帶載拉弧（§9.3）。
...
| 7 | 0–300ms：閉合湧浪（遮蔽窗內，不判過流）。🔵 **湧浪來自充電器輸出電容對電池的瞬時放電，不受 CC 環路箝制**，峰值可短暫高於 5A ⇒ `T_inrush_blank` 仍必要（實際值待 **V-8** 實測） | POWERED | 閉合 | — |
| 8 | 🔵 300ms 後 CC 環路穩定，電流箝在 **5.0A** | POWERED | 閉合 | — |
| 9 | 🔵 **約 4 小時後**進 CV（CC 段補 20Ah ÷ 5A ＝ 4.0h，§1 模型），電流緩降至 0.25A | POWERED | 閉合 | — |
| 10 | 維持 <0.3A 滿 300s，且非驟降 | **CHARGED** | **維持閉合** | T18 |
| 11 | MQTT 發 `full`，綠燈慢閃 | CHARGED | 閉合 | — |
| 12 | 機器人收到上層指令離站，刷塊退出，SW-L/SW-R 釋放 | **IDLE** | **開** | T20 |

🔵 **一輪總時長 ≈ 5.7h**（CC 4.0h + CV 尾段 1.7h，§1 模型；步驟 10 的 `T_full` 300s 已含在 CV 尾段內）。

> 🔵 **T21（CHARGED → POWERED，電流 > 2.0A 持續 5s）在 5A CC 下的裕度驗算**
>
> T21 是「充飽後電池自放電、充電器補電 ⇒ 回到充電中」的回程路徑，其成立條件是
> **補電時的電流必須能突破 2.0A 判定門檻**。
>
> | 檢查 | 算式 | 結果 |
> |---|---|---|
> | 補電電流上限 vs T21 門檻 | 5.0A ÷ 2.0A ＝ **2.50 倍** | ✅ 成立（原案 10.7A 時為 5.35 倍） |
> | 🔴 若 CC VR 沒設滿（調到下限 3A） | 3.0A ÷ 2.0A ＝ **1.50 倍** | ⚠️ 仍成立但裕度變窄 ⇒ **這是「VR 必須設滿 5A」的第二個理由** |
> | 同一門檻用於 T17（`I_leave_prev` = 2.0A） | 充電中 5.0A ＞ 2.0A | ✅ 驟降備援判定不受影響 |
>
> **物理面**：電池自放電後端電壓低於 28.0V，充電器立刻回到 CC 段輸出 5A（不是慢慢爬升），
> 因此 T21 讀到的是 **5A 而非某個介於 0.3–2.0A 的中間值**——2.0A 門檻不會落在「上不去也下不來」的死區。
> 🔴 **但 `I_float`（0.3A）與 `I_leave_prev`／T21（2.0A）之間的 0.3–2.0A 帶狀區在 5A CC 下依然是死區**，
> 這與換充電器無關（原案同樣有），且 CV 尾段本來就會掃過這個區間 ⇒ 由 `T_full` 300s 的持續條件擋掉誤判，設計不變。
...
（🔵 拉弧電流由原案的 8.5A 降為 **5.0A**，弧能量約降為 (5.0/8.5)² ≈ **35%**，刷板壽命只會更好），
K1 隨後才斷開。這是降級但不失效的路徑，**已列為待驗證項 V-4**。

**若微動開關機械卡死在 ON**：走備援路徑——刷塊分離 ⇒ 電流從 🔵 **5.0A** 驟降至 0
（🔵 仍滿足 T17 的「前一取樣 > `I_leave_prev` 2.0A」⇒ 5.0 ÷ 2.0 ＝ **2.5× 裕度**，備援判定成立）⇒
...
🔵 對 **≈5.7 小時**的充電週期（§1 模型）而言可忽略——占比 0.014%，比原案 3.6h 時更不顯著。
...
| **#2 軌失效但 ESP32 還活著**（上一列的延伸） | ESP32 會以為自己閉合了 K1（`relay_closed` = true），實際上 K1 是開的 ⇒ 🔴 **MQTT 的 `relay_closed` 欄位在此情境下失真**。**刷板無電，方向仍然安全。**<br/>充電中才壞 ⇒ 電流由 🔵 **5.0A** 掉到 0 ⇒ **T17 帶回 IDLE** ✅（5.0A ＞ `I_leave_prev` 2.0A，裕度 2.5×）；<br/>⚠️ 若**入座前就已經壞了**，電流全程為 0、無驟降 ⇒ T17 不成立，會走 T18 在 300s 後誤報 **CHARGED**（狀態失真，但 K1 從頭到尾是開的） | 分軌設計固有的「回報失真但實體安全」情境，且**與本次變更無關**（v1.1 的 24V 軌失效行為相同）。v1 接受，列為 **R-12**；不加接點回授（那是 v2 才值得的成本） |
...
| 手段 | 內容 | 覆蓋度 |
|---|---|---|
| **降額** | 🔵 **5A / 30A(30VDC) = 16.7% 負載率**（原案 10.7A 時為 36%）。接點熔接的機率隨負載率非線性上升，16.7% 已是**極保守區**——換 HLG-150H 後這一層緩解比原案更強（見 §15 決策 13、16） | 預防 |
...
**盲區在 v1 完全沒有自動化解法**（🔄 2026-08-11 裁決後的現實）：
量 **K1 下游**電壓確實能不論機器人在不在都判定熔接，但 **v1 連 TP-PROBE 排針與走線都不做**
（圖面 `J3` 已刪，§4.4），所以這件事在 v1 **只能人工量測**：

- **人工量測路徑（v1 唯一手段）**：斷開 AC 前，用萬用表 DC 檔量
  **⊕ 端子台第三位（K1 下游）↔ ⊖ 端子台**——這兩處都是 M4 環形端子的裸露螺絲，
  探棒直接夾得到，**不需要任何排針**。IDLE 狀態下讀到 ≈28V ＝ 接點熔接。
  刷板金屬面（⊕/⊖ 刷條）也量得到同一組電壓，但**端子台優先**：刷板面積大、探棒易滑跨兩極。
- **什麼時候該量**：每次進場維護、或懷疑 K1 有帶載切斷史時；**不是常態監控**——
  這正是盲區之所以是盲區的原因，v1 靠的是下面的實體緩解，不是偵測。
- **v2 的自動化解法仍然是電壓試探擴充點（§4.4）**，但施作範圍已放大為
  **走線 + 排針 + P1–P4 全部從零做起**，不再是「把料補上」而已。

**v1 期間對盲區的實體緩解（必須施作）**：
1. dock 刷板**朝下安裝或加物理遮蔽**，降低異物橫跨兩極造成短路的機率
2. 28V DC 對人體不構成電擊致命風險，但**短路電流可達 🔵 5A（HLG-150H-30A 的 CC 上限）** ⇒ 遮蔽的目的是防短路，不是防觸電。<br/>🔵 相較原案的 10.7A，短路能量降為約 (5/10.7)² ≈ **22%**，但**遮蔽仍是必須項**——5A 足以在異物橫跨兩極時產生持續電弧與高溫
3. 上游仍有 F-D1 15A 保險絲與 **AC 側 RCBO（30mA / C6 @110V）** 兜底。<br/>🔴 **誠實揭露**：5A 短路**不會**熔斷 F-D1（15A），也不會跳 C6（AC 側只反映約 1.8A 輸入）⇒ **這兩層對「刷板被異物短路」本來就無效**，靠的是充電器自身的 constant-current limiting（SPEC：`SHORT CIRCUIT — Constant current limiting, recovers automatically`）與物理遮蔽。**這一點與原案 320H 相同，不是換充電器造成的**
...
| 方向 | 順序 | 為什麼要這樣 |
|---|---|---|
| **進入** | 刷塊先接觸 → 微動後觸發 → （`T_confirm` 500ms）→ K1 閉合 | K1 閉合時接觸已完全建立，避免在接觸不穩定時送 🔵 **5A**（局部發熱、接觸電阻大）。<br/>🔵 電流降低不改變這條要求：接觸不良的發熱是 I²R，R 在接觸不穩時可跳兩個數量級，5A 一樣燒得起來 |
...
|---|---|---|---|---|---|
| **D1** | 充電器 | 🔵 **Mean Well HLG-150H-30A（2026-08-11 定案，取代原案 HLG-320H-30A）**<br/>SPEC 實值：DC 30V 檔、**CV 可調 27–33V**（電位器調至 **28.0V**，距下限有 1.0V 餘裕）、**CC 可調 3–5A（🔴 VR 設滿 5A）**、CC 區間 15–30V、額定 150W、效率 93.5%、AC 1.7A@115VAC、CC+CV、IP65。<br/>🔴 尾碼**必須是 -30A**（**A ＝ 內建電位器可調**，SPEC：`Adjustable for A/AB-Type only (via built-in potentiometer)`）。`HLG-150H-30`（無 A）固定 30V，**買錯無法使用**。<br/>🔴 **禁用任何 29.4V 固定輸出的 7S 充電器**。<br/>🔵 **到貨後兩顆 VR 都要調**：① 電壓 VR 調 **28.00V**（空載，上電接負載前）② 電流 VR **轉到底 ＝ 5A**（不設滿會拉長充飽時間並壓縮 T21 裕度，見 §7.1）。<br/>⚠️ **不是 320H**——若沿用舊版 BOM 買到 320H，電氣上仍可用（保護階梯對 10.7A 也成立），但**多花約 NT$1,950 且與本文件全文數字不符** | 1 | 蝦皮（NT$940，2026-08 實價）；廣華 <https://shop.cpu.com.tw/product/55843>（**待詢價**）；明緯經銷（良興 EcLife、唐訊） | 🔵 **NT$940**<br/><sub>（原案 320H NT$2,583–2,900）</sub> |
| **D2** | 刷板組 | AGV 小車充電板配套刷板，**20A 檔、鍍鎳**（🔵 **仍選 20A**；換 150H 後原本的「15A 檔會夾死保險絲」論證已不成立，改以接觸件降額與 F-D1 備品共用為理由，見 §3.2 與 §14 R-14）。含 dock 側刷板 + 車底彈簧刷塊。賣家已提供 3D 圖檔 | 1 組 | 樂天「青春公社」（淘寶轉賣） | NT$800–2,000<br/>**需向賣家確認** |
...
| **D20** | AC 側保護 | 🔄 **RCBO 30mA + C 曲線 C6**（漏電＋過流二合一，**取代**原 2A 慢熔）。<br/>🔴 **不可選 C2**（會被充電器開機湧浪誤跳，理由見 §3.7）。<br/>備案：上游無處加裝 RCBO 時才改用 **T5A@110V 慢熔** + 座（🔴 **不是 2A——2A 是 220V 的數字**，對定案 150H 在 110V 的 ~1.8A 只有 1.1× 裕度；🔵 **T4A 對 150H 雖也夠（2.2×），仍維持 T5A 不換料**，見 §3.7） | 1 | 五金行、電料行（士林電機／台芝／施耐德等 DIN 導軌品） | **需詢價**（RCBO 遠高於保險絲） |
...
| D27 | 排針 + 杜邦線 | 2.54mm，**母-母杜邦線 ≥4 條** | I2C、**K1 模組 `IN`（WD-22，2–4mA）**（🔄 **TP-PROBE 擴充點已自本列移除**：2026-08-11 裁決 v1 不施作排針，§4.4）。<br/>🔴 **`DC+`/`DC−` 不可用杜邦線**，走 D23 的 22 AWG（§3.3 施工要求 4） | 光華商場、露天/蝦皮 | NT$50 |
...
| — | ~~排針 **TP-PROBE-HI / LO**~~ | 2.54mm ×2 | 🔄 **0（v1 不施作）** | 🔄 **2026-08-11 裁決**：圖面 `J3` 已刪，**連排針與走線都不做**（§4.4）。列封存不刪，v2+ 裝料時連同 WD-P1/WD-P2 一併補做 |

> 🔄 **v1 這組擴充點的採購與施工量皆為零**（原文「只施作走線與排針」已隨 2026-08-11 裁決作廢）。
> 分壓比預算見 §4.4，日後裝料時直接照抄不必重算。
> 這組擴充點有**兩個**用途：①電壓試探（若日後推翻裁決 1）②接點熔接偵測（§8.3 的盲區解法，**建議 v2 優先施作**）。
> ⚠️ **v2 的起點是空板**：走線、排針、P1–P4 全部要做，不是「把料補上」。

### 11.4 成本估算

| 項目 | 金額 |
|---|---|
| 🔵 **充電器（HLG-150H-30A，定案）** | **NT$940**<br/><sub>（原案 HLG-320H-30A：NT$2,900 ⇒ **省 NT$1,960**）</sub> |
...
| **合計** | 🔵 **~NT$3,600–5,790 ＋ RCBO 詢價**<br/><sub>（原案為 ~NT$5,560–7,750；算式：5,560 − 1,960 ＝ 3,600、7,750 − 1,960 ＝ 5,790）</sub> |
...
>
> 🔵 **充電器定案對總價的影響：−NT$1,960**（HLG-320H-30A NT$2,900 → HLG-150H-30A NT$940）。
> 這是本專案目前為止**單筆最大的成本削減**，且**沒有任何一個保護件或韌體常數需要跟著改**（§3.2）。
> ⚠️ NT$940 為蝦皮 2026-08 實價；廣華 <https://shop.cpu.com.tw/product/55843> **尚未詢價**，
> 若廣華含稅價明顯較高，仍以蝦皮為準（列為 §14 R-13 的附帶待辦）。
>
...
| **V-7** | 🔵 **HLG-150H-30A 兩顆 VR 的實測收斂**：① 電壓 VR 調至 **28.00V** 後的實際輸出（含 ±1.0% VOLTAGE TOLERANCE 與 ±0.5% LOAD REGULATION）落在 `V_bus_min`/`V_bus_max` ＝ **26.0–29.5V** 內；② **電流 VR 轉到底時的實際 CC 值是否真的到 5A**（SPEC 標 3–5A 可調） | 板凳測試 | ① §6.2 的 T10 門檻，太窄會誤觸發 FAULT_`VBUS`。<br/>🔵 驗算：28.00 × (1 ± 1.5%) ＝ **27.58–28.42V**，距 26.0/29.5 兩端各有 1.58V / 1.08V ⇒ 帳面充足，但要實測確認。<br/>② 🔵 CC 值直接決定充飽時間（§1 模型）與 T21 裕度（§7.1）——**若實測只到 4.5A，充飽時間變 6.1h**，需回頭更新 §1 |
| **V-8** | K1 閉合瞬間的實際湧浪電流與持續時間 ⇒ 驗證 `T_inrush_blank` = 300ms 是否足夠 | 板凳測試（接假負載） | §6.2，太短會在每次入座時誤判過流 |
| **V-9** | 電池實際充電曲線的 CV 尾電流 ⇒ 驗證 `I_float` = 0.3A 與 `T_full` = 300s；🔵 **併驗 §1 充飽時間模型**（`t ≈ 20Ah ÷ I_cc + 1.7h`，@5A 預期 **5.7h**：CC 段 4.0h + CV 尾段 1.7h） | 首次完整充電 | §6.2 的 T18，門檻不對會永遠進不了 CHARGED。<br/>🔵 §1 的 5.7h 是**模型推算不是實測**（CV 尾段 1.7h 尤其依賴電池個體差異）；實測值若偏離 ±30% 以上，§1、§7.1、§7.4 的時間敘述都要跟著更新 |
...
| # | 問題 | 影響 | 建議處置 |
|---|---|---|---|
| **R-1** | 🔴 **接點熔接 + 機器人已離開** 的偵測盲區（§8.3） | dock 刷板可能持續外露帶 28V 而無人知曉 | v1：刷板遮蔽 + 朝下安裝；**懷疑時以萬用表人工量 ⊕ 端子台第三位 ↔ ⊖ 端子台**（§8.3，v1 無排針故無自動偵測）。<br/>**v2：施作 TP-PROBE 擴充點**（🔄 走線 WD-P1/P2 + 排針 + P1–P3，2026-08-11 裁決後為從零做起），即可在 IDLE 直接量 K1 下游電壓判定熔接 |
...
| **R-5** | 充電器 remote on/off 未使用 | 每次斷開都是帶載切斷 | 🔵 **原文有誤，已依 SPEC 更正**：HLG 的 **-A 尾碼只有「內建電位器可調」，沒有任何調光/remote 腳**（SPEC：`3 in 1 dimming function (for B/AB-Type)`）⇒ **定案的 HLG-150H-30A 做不到「斷 K1 前先關輸出」**。要有此功能得改買 **HLG-150H-30AB**（A＋B，多花錢且要多拉一組調光線）。<br/>🔵 **緩解**：CC 電流由 10.7A 降為 5A，帶載切斷的弧能量約降為 22% ⇒ **這條殘留的嚴重度隨定案而下降**。v1 維持不採用，列為 v2 改善項（若採用需一併改型號為 -AB） |
...
| **R-12** | 🆕 **LM2596 #2 在入座前就失效時，狀態機會在 300s 後誤報 `CHARGED`**（§8.2 末列）：K1 實際從未閉合、刷板全程無電，但 `relay_closed` 與 `state` 欄位失真 | **實體安全不受影響**（刷板無電）；影響的是上層看到的狀態。充電中才失效的情境由 T17 正確涵蓋 | v1 接受並明文揭露。**與本次變更無關**（v1.1 的 24V 軌失效行為相同）。<br/>v2 的解法有二：① TP-PROBE 施作後在 POWERED 檢查 K1 下游是否真的有電壓（與 R-1 的熔接偵測共用同一組走線與料，🔄 2026-08-11 裁決後含排針與走線）；② 加接點回授。①成本更低且一石二鳥，建議與 R-1 一併處理 |
| **R-13** | 🆕 🔵 **充電倍率降為 0.167C、充飽時間 5.7h**，低於 §1 原訂 0.2–0.3C（6–9A）目標 | 單輪充電由 3.6h 拉長為 **5.7h**。dock 為常時停靠浮充架構，v1 使用情境下不痛；但**若日後電池加大、或改為多班輪替使用**，這會是**第一個撞到的限制** | v1 接受（§0.2 裁決 7 已明列為已知代價）。<br/>**若日後不夠用**：升級路徑是換 **HLG-240H-30A（4–8A）** 或回到 **HLG-320H-30A（5.35–10.7A）**——🔴 兩者的 CC 電流都**仍在既有保護階梯之下**（軟切 12A），⇒ **換上去不需要改任何保護件或韌體常數**，只需把本文件的裕度數字改回。<br/>⚠️ 附帶待辦：廣華 <https://shop.cpu.com.tw/product/55843> 尚未詢價，目前以蝦皮 NT$940 入帳 |
| **R-14** | 🆕 🔵 **刷板 20A 檔的原始選型論證已隨定案失效** | 原論證「15A 刷板 ⇒ 保險絲降 10A ⇒ 低於 CC 上限 10.7A 會誤熔斷」在 5A 下不再成立 ⇒ **15A 檔在電氣上變成可行選項**。<br/>影響：D2 尚未下單（數量 0，等詢價），若 15A 檔明顯便宜，目前沒有硬理由擋 | v1 **維持 20A**（理由已改寫，見 §3.2、§15 決策 2）。<br/>🔴 **若採購時發現 15A 檔價差顯著，這是使用者的採購裁決，不是本文件可自行改的**——改選須連帶把 F-D1 降到 10A、放棄與車上 F1 的備品共用，並回頭改 §3.2／§11.1 D4／§15 決策 2 |
...
| 2 | 刷板選 **20A 檔**，保險絲 **15A** | ~~15A 刷板會逼保險絲降到 10A，低於充電器 CC 上限 10.7A ⇒ 滿載誤熔斷~~<br/>🔵 **原理由已隨決策 16（改用 5A 的 HLG-150H）失效**——10A 保險絲對 5A 不會誤熔斷。<br/>**決策維持不變，理由改為**：① 刷板是接觸件，20A 對 5A 是 **4× 降額**，直接換成接觸電阻與壽命；② F-D1 15A 與車上 F1 同族、備品共用（§11.1 D4），降到 10A 就破壞共用。詳見 §3.2、§14 R-14 | 15A 刷板（🔵 **現在電氣上可行，但採購上未改選**） |
...
| **16** | 🔵 **充電器定案 HLG-150H-30A（5A），取代原案 HLG-320H-30A（10.7A）** | 使用者 2026-08-11 拍板（§0.2 裁決 7）。**功率夠用**：28.0V × 5A ＝ 140W ≤ 額定 150W，且 28.0V 落在 SPEC 的 CC 區間 15–30V 內、CV 可調 27–33V 涵蓋 28.0V（餘裕 1.0V）；**省錢** NT$1,960。<br/>🔴 **工程成本為零的關鍵**：保護階梯的不等式鏈只有最底層與充電器有關，把 10.7A 換成 5A **只讓所有裕度變大**（12A 軟切 1.12× → **2.40×**、F-D1 1.40× → **3.00×**、K1 負載率 36% → **16.7%**）⇒ **保護件與韌體常數全部不動**（§3.2）。<br/>代價：充飽 3.6h → **5.7h**、倍率 0.28C → **0.167C**（§14 R-13）；且 §15 決策 2 的原始理由失效（§14 R-14） | **HLG-320H-30A**（原案，餘裕最大但貴 NT$1,960）；**NPB-360-24TB**（NT$2,000，但 2-stage 曲線是否充飽斷電未書面確認）；**HLG-240H-30A**（🔴 CV 可調下限恰為 28.0V、無向下餘裕，SPEC 一查就否決）；淘寶訂製 28.0V（無安規、無 datasheet，且不比 NT$940 便宜） |
...
### 15.4 決策 16 詳述：充電器定案 HLG-150H-30A

**變更內容**：充電器由原案 **HLG-320H-30A**（10.7A，NT$2,583–2,900）改為
🔵 **HLG-150H-30A**（**5A**，NT$940）。使用者 2026-08-11 拍板（§0.2 裁決 7）。

**SPEC 依據**（`HLG-150H-SPEC.PDF`，HLG-150H-30 欄）：

| SPEC 欄位 | 值 | 對本設計的意義 |
|---|---|---|
| `DC VOLTAGE` | 30V | 名目檔位；實際用 VR 調到 28.0V |
| `VOLTAGE ADJ. RANGE` | **27 ~ 33V**（`Adjustable for A/AB-Type only`） | 28.0V 在範圍內，**距下限 1.0V 餘裕** ⇒ 尾碼**必須是 -30A** |
| `CONSTANT CURRENT REGION` | **15 ~ 30V** | 28.0V 落在 CC 區間內 ⇒ 5A 恆流成立，不會提前掉出 CC 段 |
| `RATED CURRENT` / `CURRENT ADJ. RANGE` | **5A** / **3 ~ 5A** | 充電 CC 電流；🔴 VR 設滿 5A |
| `RATED POWER` | **150W** | 驗算：28.0V × 5A ＝ **140W ≤ 150W** ✅（餘裕 7%） |
| `AC CURRENT (Typ.)` | **1.7A / 115VAC** | AC 保護裕度（§3.7）：T5A ＝ 2.9×、C6 ＝ 3.5× |
| `INRUSH CURRENT (Typ.)` | **65A, twidth 425µs @230VAC** | 110V 下 ~31A；脈寬比 320H 短一半以上 ⇒ C6 誤跳風險更低 |
| `MAX. No. of PSUs on 16A CIRCUIT BREAKER` | **7 units (type C) @230VAC** | 湧浪不會讓 C 型脫扣的**直接旁證** |
| `EFFICIENCY (Typ.)` | 93.5% | AC 側熱耗與電費，非設計約束 |

**為什麼工程成本是零**（這是本決策最值得記錄的部分）：

保護階梯是一條不等式鏈——`正常電流 < 軟體警告 < 軟體切斷 < 硬體保險絲 < 元件額定`。
其中**只有最左端跟充電器有關**。把 10.7A 換成 5A，等於把鏈條最左端往左推，
**右邊四層的相對位置一格都沒動**：

| 檢查 | 原案 10.7A | 🔵 定案 5A | 變化 |
|---|---|---|---|
| 軟體切斷 12A ÷ 充電電流 | 1.12× | **2.40×** | 更寬 ✅ |
| F-D1 15A ÷ 充電電流 | 1.40× | **3.00×** | 更寬 ✅ |
| K1 30A 負載率 | 36% | **16.7%** | 更保守 ✅ |
| 分流器滿刻度使用率 | 33% | **15.3%** | 仍有 5000 階解析度 ✅ |
| F-D1 15A ÷ 軟體切斷 12A | 1.25× | **1.25×** | 不變（與充電器無關） |
| 刷板 20A ÷ F-D1 15A | 1.33× | **1.33×** | 不變（與充電器無關） |

⇒ **沒有任何一格由 ✅ 變 ❌，因此保護件、韌體常數、線徑全部不動。**

**誠實記錄兩個「變寬鬆反而要說明」的地方：**

1. 🔴 **軟體過流門檻在 5A CC 下實務上不會觸發**。這不是缺陷——它保護的標的是
   「充電器 CC 環路失效」與「車端防倒灌失效導致電池回灌」，那兩種情境下 12A/14A 照常動作（§3.2）。
2. 🔵 **§15 決策 2（刷板選 20A）的原始理由失效**。原論證依賴「10A 保險絲 < 10.7A 會誤熔斷」，
   在 5A 下不成立 ⇒ 決策維持但**理由已改寫**，並列為 §14 R-14 供採購時複查。

**代價**（拍板時已知並接受）：充飽時間 3.6h → **5.7h**、充電倍率 0.28C → **0.167C**（§14 R-13）。
dock 是常時停靠浮充架構，這個代價換 NT$1,960，判斷成立。

**受本變更影響的章節**：§0（版本標頭）、**§0.2（裁決 7 新增）**、**§1（全節改寫，新增充飽時間模型）**、
§2.1（方塊圖）、§3.2（裕度表重算＋兩段新論證）、§3.3（K1 負載率）、§3.4（14 AWG 載流說明）、
§3.5（分流器驗算重算）、§3.7（AC 表改為 150H 定案、旁證換 150H SPEC）、
§7.1（電流/時間/**T21 裕度驗算新增**）、§7.2、§7.4、§6.5、§8.2、§8.3、§9.3、
§11.1（**D1 全列改寫**、D2、D20）、§11.4（成本）、§13（V-7/V-9）、
§14（**R-5 依 SPEC 更正**／**R-13、R-14 新增**）、§15（決策 2 理由改寫、**決策 16 新增**、**§15.4 新增**）、§16。
...
| §15 第二階段——無線自動充電 | 🔵 **已由使用者裁決廢棄**（不只是「被本文件取代」）：充電機構定案為接觸式刷板，無線感應耦合方案**不再是候選路徑**。<br/>`power_design.md` §15 已加上廢棄標記，詳見該檔；本文件為接觸式方案的權威來源 |
