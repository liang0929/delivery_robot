...
> AC 保護由「2A 慢熔」改為 **RCBO 30mA + C 曲線 C6**（滿載 AC 依 SPEC：HLG-150H 1.7A／HLG-320H 3.5A @115VAC，2A 會熔斷）。
...
| AC 安全（沿用 `power_design.md` §15.7，**必須項非建議項**） | ① 裝入封閉外殼，AC 端子不得可觸及<br/>② 🔴 **AC 側裝 RCBO 30mA + C 曲線 C6**（漏電＋過流二合一，**取代**原本的機內 2A 慢熔）<br/>③ 金屬外殼接 FG<br/>④ 若仍要保留機內慢熔作為備案，額定寫 **T5A@110V**；🔴 **2A 只適用 220V，110V 下會誤熔斷**；🔴 **T4A 也不夠**（對 HLG-320H 滿載 3.5A 僅 1.14×，理由見下表） | dock 放在地面、有人會經過；漏電保護在地面潮濕環境是必須項，不是建議項 |

> **🔴 為什麼從「2A 慢熔」改成「RCBO 30mA + C6」**（本節數字全部以 110V 為前提）
>
> | 項目 | 值 | 說明 |
> |---|---|---|
> | 充電器滿載 AC 輸入電流（官方 SPEC 實值） | **HLG-150H：1.7A**<br/>**HLG-320H：3.5A**<br/><sub>（AC CURRENT Typ. @115VAC）</sub> | 兩款都列，因**充電器型號尚未拍板**（§10.1）。SPEC 的 AC CURRENT 是**系列共用 typ 值**（同系列各輸出電壓版本共列一個數字），**110V 下略高**：1.7A → ~1.8A、3.5A → **~3.6A**（依 115/110 反比換算）。<br/>同一顆在 230V 下不到一半（150H 0.75A／320H 1.65A）⇒ **原本的 2A 慢熔是以 220V 算出來的**，直接搬到 110V，320H 滿載即 3.5A、**必熔斷**。<br/>（先前文中的「~3A @110V」是估值，已以官方 SPEC 實值取代） |
> | 慢熔備案額定 | **T5A**（🔴 **不是 T4A**） | 裕度：對 150H 1.7A ＝ **2.9×**、對 320H 3.5A ＝ **1.4×**，兩款都夠。<br/>🔴 **為什麼不用 T4A**：對 320H 只有 **1.14×**。慢熔的連續使用慣例要降額 ~25%（T4A ⇒ 可連續帶 3.0A ＜ 320H 的 3.5A），且 IEC 60127-2 只保證「1.5×In 一小時內不熔斷」，1.14× 雖在保證帶內卻已無降額餘裕，長時間滿載會讓熔絲慢性劣化、提早熔斷。T5A 降額後 3.75A ＞ 3.5A，才站得住 |
> | RCBO 漏電動作值 | **30mA** | 人身保護等級；dock 在地面、金屬外殼接 FG，漏電才是這個場景的主風險 |
> | RCBO 曲線與額定 | **C 曲線 / C6** | 過流裕度：對 150H 1.7A ＝ **3.5×**、對 320H 3.5A ＝ **1.7×**（以 110V 實值 ~3.6A 計仍有 1.67×），兩款都夠。🔴 **不可選 C2**：C2 的磁脫扣區間（5–10×In ＝ 10–20A）低於充電器冷開機湧浪，會誤跳；C6 的磁脫扣區間是 **30–60A**（10–20×In），湧浪峰值雖落在此區間內仍**不會**誤跳，理由見下兩列 |
...
>
> **AC 電流／湧浪資料來源**：Mean Well 官方 SPEC PDF（`meanwell.com/Upload/PDF/HLG-150H/HLG-150H-SPEC.PDF`、
> `.../HLG-320H/HLG-320H-SPEC.PDF`，INPUT 段 `AC CURRENT (Typ.)` 與 `INRUSH CURRENT (Typ.)` 兩列）。
> ⚠️ 若現場實測仍在冷開機瞬間跳脫，退路是升到 **C10**（磁脫扣 50–100A）；
> 代價是過流保護變鬆——C10 對滿載 ＝ **5.9×In（150H 1.7A）／2.9×In（320H 3.5A）**，等於 AC 側幾乎只剩漏電保護在把關，
> 因此**先用 C6，跳了再升**，不要一開始就選 C10。
>
> RCBO 一顆同時提供漏電與過流保護，**功能上涵蓋原本的機內慢熔**，所以是取代而非疊加。
> 保留慢熔備案的唯一理由是「現場只有普通插座、上游沒有可加裝 RCBO 的配電位置」——
> 走這條路時額定務必用 **T5A**，不要照抄舊文件的 2A，也不要用 T4A（對 HLG-320H 僅 1.14×）。
...
| **D20** | AC 側保護 | 🔄 **RCBO 30mA + C 曲線 C6**（漏電＋過流二合一，**取代**原 2A 慢熔）。<br/>🔴 **不可選 C2**（會被充電器開機湧浪誤跳，理由見 §3.7）。<br/>備案：上游無處加裝 RCBO 時才改用 **T5A@110V 慢熔** + 座（🔴 **不是 2A——2A 是 220V 的數字**；🔴 **也不是 T4A**——對 HLG-320H 滿載 3.5A 僅 1.14×，見 §3.7） | 1 | 五金行、電料行（士林電機／台芝／施耐德等 DIN 導軌品） | **需詢價**（RCBO 遠高於保險絲） |
