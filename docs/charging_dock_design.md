...
> 版本：v1.1（設計定案，未施工）
> 最後更新：2026-08-07
> 🔄 **v1.1 變更**：主繼電器 K1 改用 icshop 光耦繼電器模組（24V 版），取代 v1 的裸繼電器 + 自焊驅動電路。
> 模組採**分離供電模式**（拆除 DC/JD 兩個短接跳線帽：線圈側吃 24V、觸發側吃 5V），H/L 跳線設 **H**（高電平觸發）。
> 詳見 **§15 決策記錄 13、14**；新增 **§3.10 到貨驗收清單（A-1 ~ A-4）**，其中 **A-1 為 §8 fail-safe 論證的前置條件**。
...
flowchart LR
    subgraph DOCK["充電 Dock（獨立基礎設施）"]
        ...
        K1["K1 繼電器模組（光耦隔離）<br/>Songle SLA-24VDC-SL-C<br/>分離供電：JD±=24V 線圈 / DC±=5V 觸發<br/>只接 COM+NO，NC 空置禁用<br/>30A / 30VDC"]
        BR["刷板 20A 檔<br/>鍍鎳"]
    ...
    CHG --> BK1 --> ESP
    BK1 -->|"5V → 模組 DC+/DC−（觸發側）"| K1
    CHG --> BK2 -->|"24V → 模組 JD+/JD−（線圈側）"| K1
...
```
                     ...
                     │               └►│ DC+ ● 觸發側（5V 域）                │
                     │                 │ DC− ● ──► ESP32 GND                  │
                     │                 │ IN  ●◄─── GPIO25（3.3V HIGH = 吸合） │
                     │                 │ H/L 跳線 ＝【H】高電平觸發           │
                     │                 │ ══════ 光耦（輸入↔線圈隔離）══════   │
                     ├──[LM2596 #2]───►│ JD+ ● 線圈側（24V 域）               │
                     │     24.0V       │ JD− ● ──► ⊖ 端子台                   │
                     │                 │ [板載驅動級 + 飛輪二極體] → 線圈     │
                     │                 │ 🔴 DC/JD 兩顆短接跳線帽【已拆除】    │
...
充電器 ⊖ ══[12AWG]══► ⊖ 端子台（dock 單點接地）══[12AWG]══► 刷板 ⊖
                            ...
                            ├── 繼電器模組 JD−（線圈側回線，24V 域）
                            ├── 繼電器模組 DC−（觸發側回線，接 ESP32 GND ⇒ 同一星點）
                            └── 微動開關 ×2 的回線

  ⚠️ 只使用 **COM + NO**：光耦無電流 ⇒ 線圈失電 ⇒ COM–NO 開路 ⇒ 刷板無電。fail-safe 依據見 §8。
  ⚠️ **分離供電不是電源隔離**：光耦確實隔離觸發側與線圈側，但 dock 上兩側的回線
     （DC− 與 JD−）最終都回到同一個 ⊖ 星點 ⇒ 拓樸上隔離、應用上共地（§3.7）。
...
**選定：icshop「30A 5V/12V/24V 繼電器模組-帶光耦」的 24V 版（現成模組，非裸繼電器）**

- 商品連結：<https://www.icshop.com.tw/products/368030501112>，NT$79/片
- 板上繼電器本體：**Songle SLA-24VDC-SL-C**（本體印字 **30A 250VAC / 30VDC**）
- 板上另有：光耦（輸入與線圈電氣隔離）、板載驅動級與飛輪二極體、輸出螺絲端子台（標「常閉／公共／常開」），
  以及**兩組功能完全不同的跳線**——`DC+/JD+` 與 `DC−/JD−` 的**分離供電短接帽 ×2**、`H/L` 的**觸發極性跳線 ×1**（詳見 §3.4）
...
| 線圈電壓 | 24VDC | 24.0V（LM2596 #2 → **JD+**） | — |
| 線圈側耗流（JD±） | 線圈 ~38mA + 板載驅動級 ⇒ **估 ~40mA** | ~40mA | LM2596 #2 額定 3A ⇒ ~**75×** |
| 觸發側耗流（DC±） | 光耦 LED + 板上指示 LED ⇒ **估 ~10–20mA** | ~20mA（取上限） | 掛 5V 軌，見 §3.6 |
| 輸入介面 | 光耦隔離，**觸發側由 DC± 獨立供電**（分離供電模式）；觸發電流 **2–4mA**、高電平門檻 **2.5–5V**〔5V 版數據，24V 版待 A 系列確認〕 | GPIO25 3.3V 直推 | 裕度 0.8V，見 A-2 |
...
4. 低壓側共五條線（**JD+/JD−**（24V，~40mA）、**DC+/DC−**（5V，~20mA）、**IN**）可走洞洞板銅箔或杜邦線；
   🔴 **接線前先確認 DC/JD 兩個短接跳線帽已拆除**（A-4），短接模式下 JD 與 DC 同電位，5V 與 24V 兩軌會被跳線帽直接短在一起
5. 模組建議以螺柱獨立固定，與 ESP32 主板分開，避免功率線橫越訊號區
...
回到裸繼電器 SLA-24VDC-SL-**A** ＋ 自焊驅動電路（Q1 2N2222A / R1 1kΩ / R2 10kΩ / D1 1N4007），
...
| 跳線 | 功能 | 商品頁原文 |
|---|---|---|
| **`DC+/JD+` 與 `DC−/JD−`**（兩顆跳線帽） | **分離供電跳線**。短接 ⇒ 觸發端與線圈端同電壓；拆除 ⇒ 兩級各自獨立供電 | 「DC+ 和 JD+ 用跳線帽短接，DC− 和 JD− 用跳線帽短接，觸發端與繼電器控制端電壓相同」；拆開則「可做到前後兩級不同電壓完全隔離（如繼電器用 24V，觸發用 5V）」 |
| **`H/L`**（一顆跳線帽） | **觸發極性切換**，與供電無關 | 「跳線與 L 端連接時，IN 端為低電平觸發；與 H 端連接時為高電平觸發」 |

> ⚠️ **本文件 v1.1 早期版本把「JD 跳線」寫成高/低電平觸發切換，是錯的**，已全文更正。
> 更正依據與誤解成因記在 §15 決策記錄 **14**。

#### 3.4.2 🔴 為什麼主方案必須用分離供電（短接模式禁用）

**24V 版在短接模式下，觸發端電位就是 24V，板上限流電阻是按 24V 選的。**
此時 ESP32 用 3.3V 直推 IN，灌進光耦 LED 的電流只有設計值的一個零頭
（同一顆限流電阻，電流大致按驅動電壓等比縮放 ⇒ 3.3/24 ≈ **14%**，
遠低於觸發所需的 2–4mA）⇒ **A-2 幾乎必不過**。

拆掉兩顆短接跳線帽、把 **DC+ 接 5V 軌**之後，觸發端回到與 5V 版相同的電壓域，
3.3V 對「高電平 2.5–5V」的門檻才是合法輸入。

> 🔴 **分離供電是必要條件，但不保證充分——這一點要誠實講。**
> 若該片板子的 IN 路徑限流電阻是**按 24V 焊死**的（而不是隨 DC+ 電壓決定工作點），
> 那麼把 DC+ 降到 5V 並不會改變 IN 路徑上的電阻值，3.3V 直推仍然灌不進 2–4mA。
> 換句話說：**短接模式一定不過；分離供電則是「有機會過」，過不過只有 A-2 能定生死**。
> 這正是 §3.4.4 推定 1／2 要實測收斂的核心問題，也是 A-2 步驟⑥ 要串電流表量 IN 端電流的原因。

⇒ **施工紀律：兩顆 DC/JD 短接跳線帽必須拆除（A-4），拆下的跳線帽裝袋貼標保管**
（退回短接模式或退貨時要還原）。**跳線帽留在板上 = 觸發端變 24V = A-2 必不過**；
更嚴重的是，此時 JD+ 已接 24V、DC+ 已接 5V，跳線帽會把**兩條電源軌直接短路**。

#### 3.4.3 接線與設定

```
   +5.0V  (LM2596 #1 OUT+，與 ESP32 VIN 同軌) ──► ● DC+ ┐ 觸發側（5V 域）
   ESP32 GND ────────────────────────────────► ● DC− │
   ESP32 GPIO25 ─────────────────────────────► ● IN  │  IN ─[R_lim]─▶|─ 光耦 LED
                                                     │
   H/L 跳線 ＝ 【H】高電平觸發 ──────────────────────┘
   🔴 DC+/JD+、DC−/JD− 兩顆短接跳線帽 ── 【拆除】
                            ═══ 光耦（電氣隔離）═══
   +24.0V (LM2596 #2 OUT+) ──────────────────► ● JD+ ┐ 線圈側（24V 域）
   ⊖ 端子台 (dock 星點) ─────────────────────► ● JD− │  板載驅動級 ─► 線圈
                                                     │  線圈兩端已並聯板載飛輪二極體
                                                     ┘
...
**模組共 7 個接線點**：低壓側 5 條（JD+ / JD− / DC+ / DC− / IN）＋ 功率側 2 條（COM / NO），**NC 空置**。

| 介面 | 接到 | 說明 |
|---|---|---|
| **JD+** | LM2596 #2 OUT+（**24.0V**） | 線圈側供電（線圈 + 板載驅動級），~40mA |
| **JD−** | ⊖ 端子台（dock 星點） | 線圈側回線 |
| **DC+** | LM2596 #1 OUT+（**5.0V**，與 ESP32 VIN 同軌） | 觸發側供電（光耦 LED + 指示 LED），~10–20mA。§3.6 已把這筆負載計入 5V 軌 |
| **DC−** | **ESP32 GND** | 觸發側回線。ESP32 GND 本身也在 ⊖ 星點上（§3.7） |
| **IN** | ESP32 **GPIO25** | 高電平觸發：HIGH = 吸合，LOW/開路 = 釋放 |
| **`H/L` 跳線** | — | 🔴 **必須設在 `H`（高電平觸發）位**（驗收項 **A-4**）。設成 `L` 會讓 §8 的 fail-safe 方向反轉 |
| **`DC/JD` 短接跳線帽 ×2** | — | 🔴 **必須拆除**（驗收項 **A-4**）。留在板上見 §3.4.2 |
| **COM / NO** | 功率路徑 | 見 §3.3 施工要求與 §4.1 |
| **NC** | **不接** | 🔴 空置並貼標籤 |

#### 3.4.4 觸發端裕度分析（決定 A-2 為什麼必測）

**規格（商品頁 5V 版數據）**：觸發電流 **2–4mA**、高電平觸發 **2.5–5V**、低電平 **0–1.5V**。
...
| 觸發電流 | 需 2–4mA；ESP32 單腳最大 40mA（建議 ≤20mA） | 電流能力綽綽有餘 ✅ |
| 低溫／元件離散／LM2596 5V 軌誤差 | 三者疊加可能吃掉 0.3–0.5V | **這就是 A-2 要逼出來的邊緣行為** |

**🔴 兩個推定，必須在到貨驗收時確認（本節數據不是 24V 版的實測值）：**

| # | 推定 | 若不成立的後果 | 由誰確認 |
|---|---|---|---|
| 1 | **24V 版在分離供電下，觸發端規格等同 5V 版**（2–4mA / 2.5–5V）——亦即三個電壓版本共用同一片觸發端電路，只有繼電器本體不同 | 若 24V 版的 IN 端限流電阻是按 24V 選的（而非按 DC+ 電壓選），DC+ 改接 5V 也救不了 3.3V 直推，I_F 仍遠低於 2–4mA | **A-2**（實測吸合可靠度）＋ **V-2**（實測 IN 端電流／推算 R_lim） |
| 2 | 觸發端的 LED 電流路徑為「IN → 限流電阻 → 光耦 LED → DC−」（H 模式由 IN 灌入） | 若實際是由 DC+ 供電、IN 只當開關，則 DC+ 的電壓才是決定因素，裕度分析要重算 | 同上 |

> **原 v1.1 的「IN 串 1kΩ ⇒ I_F = 2.1mA」估算已作廢**——那是 5V 版的典型值推定，
> 對 24V 版沒有依據。本設計改為**只引商品頁明列的觸發端規格（2–4mA / 2.5–5V）**，
> 並把「這組數據能不能套到 24V 版」誠實標為推定，交由 A-2/V-2 實測收斂。
> 規格書與商品頁都回答不了的事，就不要在文件裡假裝算得出來——
> 所以 **A-2 是安裝前置條件，不是選配**。
...
|---|---|---|
| 輸出 | **5.0V** | **24.0V** |
| 負載 | ESP32-WROOM-32 DevKit（VIN 腳）<br/>**＋ K1 模組觸發側 DC+**（光耦 LED + 指示 LED，分離供電） | K1 繼電器模組 **JD+**（線圈 + 板載驅動級） |
| 負載電流 | ESP32 平均 ~150mA／WiFi TX 峰值 ~500mA<br/>**＋ 模組觸發側 ~10–20mA**<br/>⇒ 平均 **~170mA**、峰值 **~520mA** | ~40mA |
| 模組額定 | 3A（實際 2A） | 3A |
| 餘裕 | **~3.8×**（2A / 520mA） | **~75×** |
| 廢熱 | 28→5V 效率約 70% ⇒ 峰值 ~1.15W、平均 ~0.36W ⇒ **建議加小散熱片** | 28→24V 降壓比小，效率 >90% ⇒ <0.1W，免散熱 |

> **5V 軌新增負載的來源**：分離供電模式把模組觸發側（DC+）從 24V 軌移到 5V 軌（§3.4.3）。
> 這筆 ~10–20mA 的估值與 §3.4.4 的觸發電流 2–4mA **不衝突**——
> 2–4mA 是流過 IN 端光耦 LED 的電流（由 GPIO25 供給），
> 10–20mA 則是 DC+ 供給觸發側的總電流（含板上電源／狀態指示 LED，通常各 5–10mA）。
> 兩者路徑不同，V-2 會分別實測。餘裕從 4× 掉到 ~3.8×，對結論無影響。
...
> **分離供電沒有破壞這個理由。** 掛到 5V 軌的是**觸發側**（DC+，~10–20mA 的直流小負載，
> 且是 LED 類阻性負載，沒有湧浪）；**線圈的電感性湧浪仍完全留在 24V 軌**（JD+），
> 兩者之間隔著光耦。分軌理由不變，而且比短接模式更乾淨——短接模式下觸發端與線圈端同電位，
> 湧浪造成的軌上壓降會直接出現在 IN 的參考電位上。
...
**K1 繼電器模組 JD−（線圈側）**；**模組 DC−（觸發側）接 ESP32 GND**，而 ESP32 GND 本身即在星點上。

> 🔴 **分離供電 ≠ 電源隔離，不要寫過頭也不要讀過頭。**
>
> | | 事實 |
> |---|---|
> | **拓樸上** | 拆掉 DC/JD 短接跳線帽後，觸發側（5V 域）與線圈側（24V 域）之間**只剩光耦這條光學路徑**，兩級的供電與回線在模組上確實是分開的 ✅ |
> | **應用上** | dock 只有一個星點：DC− 經 ESP32 GND、JD− 直接，**最終都回到同一個 ⊖ 端子台** ⇒ 兩側共地，**不是 galvanic isolation** ❌ |
>
> 換句話說：**分離供電買到的是「電壓域分開」（讓 3.3V 對得上觸發門檻），不是「地分開」。**
> 光耦在本設計的真正價值是**驅動級隔離與 fail-safe 結構**（§8），
> 若哪天真要做兩地隔離，DC− 就不能再回星點，那是另一個設計，不在 v1 範圍。
...
>
> **測試環境（🔴 分離供電模式，與短接模式不同，不要照舊做法接）**：
> 模組單獨在板凳上測，**功率端（COM/NO）完全不接線**，只接 JD+/JD−/DC+/DC−/IN。
>
> | 項目 | 設定 |
> |---|---|
> | **DC/JD 兩顆短接跳線帽** | 🔴 **先拆除**（拆下後裝袋貼標保管）。A 系列全部在分離供電模式下執行 |
> | **`H/L` 跳線** | 🔴 **設在 `H`（高電平觸發）** |
> | 線圈側 | JD+ ← 24V（LM2596 #2 或任何 24V 電源）、JD− ← 電源 ⊖ |
> | 觸發側 | DC+ ← **5V**（LM2596 #1 或任何 5V 電源）、DC− ← **5V 電源 ⊖，且與 24V 電源 ⊖ 共接**（複現 dock 上的共地條件，§3.7） |
> | 判定方式 | **聽繼電器吸合聲 + 三用電表電阻檔量 COM–NO 通斷**雙重確認，不要只看板上指示 LED（LED 亮不代表接點真的吸合） |
>
> ⚠️ 板凳測試若只有一組電源，**寧可先借一顆 5V 電源也不要用短接模式湊合**——
> 短接模式測出來的結論（尤其 A-2）對主方案沒有參考價值（§3.4.2）。

| # | 項目 | 步驟 | 預期結果 | 不過的話 |
|---|---|---|---|---|
| **A-1** | 🔴 **浮空測試**（fail-safe 前提） | ① 先完成 **A-4**（兩顆短接跳線帽已拆、`H/L` 在 `H`）<br/>② JD+/JD− 接 24V、DC+/DC− 接 5V（兩電源 ⊖ 共接），**IN 端完全懸空**（不接任何線）<br/>③ 上電，靜置 10s，期間用手輕拍模組、以帶電導線在 IN 附近 5cm 掃過（模擬雜訊耦合）<br/>④ 量 COM–NO 電阻 | **K1 不吸合**：無吸合聲、COM–NO 為**開路（∞Ω）**，全程 10s 皆然 | 🔴 **停止安裝。** 表示模組板上有預設拉高電路或跳線設定不符，§8 的結構論證不成立。改測 IN 對 DC− 加 10kΩ 下拉後是否通過；仍不過則退回裸繼電器方案（§15 #3） |
| **A-2** | **3.3V 驅動測試**（分離供電下） | ① 同上供電<br/>② 以 ESP32 GPIO25（或任何 3.3V 邏輯輸出）接 IN，輸出 HIGH<br/>③ 確認吸合後，**反覆切換 HIGH/LOW 至少 20 次**，每次停留 1s<br/>④ 把 **24V 側降到 22V**（LM2596 誤差下限）重測 5 次<br/>⑤ 把 **5V 側降到 4.75V**（同上）重測 5 次<br/>⑥ 🔴 **量測並記錄**：吸合時 IN 端對 DC− 的電壓、**串電流表量 IN 端灌入電流**（驗證是否落在 2–4mA）、DC+ 與 JD+ 各自的耗流 ⇒ 填入 **V-2** | **每一次都可靠吸合與釋放**，無漏吸、無延遲吸合、無抖動；IN 端電流量到 **2–4mA** 量級（⑥ 同時收斂 §3.4.4 的兩個推定） | 🔴 **優先退回裸繼電器方案**（§15 決策記錄 #3）。**只有在裸繼電器買不到（缺料／停產）時**，才改走下方「A-2 fallback：2N2222A 緩衝」——理由見該段開頭的偏好序說明 |
| **A-3** | **本體印字確認** | 目視繼電器本體上蓋印字 | 型號欄為 **SLA-24VDC-SL-C**（🔴 電壓欄是 **24**，不是 5 或 12）；額定欄同時有 **30A 250VAC** 與 **30A 30VDC**，🔴 **30VDC 那一列必須存在** | 只有 AC 額定 ⇒ **不可採用**（§3.3 的 DC 額定紀律）。買到 5V/12V 版 ⇒ 不可改接 24V，退換貨 |
| **A-4** | 🔴 **跳線確認（兩組都要看）＋ 短接模式禁用** | ① 目視 `DC+/JD+` 與 `DC−/JD−` 兩處：**兩顆短接跳線帽都已拔除**，拔下的跳線帽裝袋貼標「K1 模組 DC/JD 短接帽 — 分離供電下不得裝回」<br/>② 目視 `H/L` 跳線，對照板上絲印<br/>③ 上電前以電表電阻檔確認 **DC+ ↔ JD+ 為開路**、**DC− ↔ JD− 為開路**（目視可能被跳線座殘留誤導） | ① 兩顆短接跳線帽**皆不在位**<br/>② `H/L` 跳線在 **`H`（高電平觸發）**位<br/>③ DC+ ↔ JD+、DC− ↔ JD− 皆為開路 | 跳線帽未拆 ⇒ **拆除後重跑 A-1 與 A-2**。🔴 **絕對不可在跳線帽仍在位的情況下接線上電**：JD+ 已接 24V、DC+ 已接 5V，跳線帽會把**兩條電源軌直接短路**（§3.4.2）。`H/L` 位置錯 ⇒ 撥到 `H` 後同樣重跑 A-1 與 A-2 |
...
```
   ⚠️ 採用此 fallback 時，`H/L` 跳線必須改設【L】低電平觸發
      （DC/JD 兩顆短接跳線帽仍然拆除，分離供電不變）

                                       ● DC+ ── +5.0V（LM2596 #1）
   GPIO25 ──[R1 1kΩ]──┬── B            ● DC− ── ESP32 GND
                      │   ┌───────┐    ● JD+ ── +24.0V（LM2596 #2）
...
- **為什麼這樣能解決問題**：`L` 模式下，光耦 LED 的電流由模組板上的 **DC+（本設計為 5V）**
  經內部限流電阻供給，**不再受 ESP32 3.3V 高電平的天花板限制**——
  觸發端回到 5V 版的原廠工作點；2N2222A 只負責把 IN 拉到地，是純開關用途（Ic 遠低於 600mA 額定）。
...
**分離供電多出來的一級退路：DC+ 改接更高電壓（🔴 偏好序更後面，非優先）**

分離供電讓 DC+ 成為一個**可調的設計參數**（短接模式下它被綁死等於線圈電壓），
因此在「裸繼電器缺料 ⇒ 走 2N2222A fallback」之後，還多了一級可調空間：

| 退路 | 做法 | 適用時機 | 代價與限制 |
|---|---|---|---|
| **主方案** | 分離供電 + `H` + GPIO25 直推 | A-2 通過 | 無 |
| **退路 1（優先）** | 退回裸繼電器方案（§15 #3） | A-2 不過 | 需自焊 Q1/R1/R2/D1；但 fail-safe 論證完全自主 |
| **退路 2** | 2N2222A 緩衝 + `L` 模式，DC+ 維持 **5V** | 退路 1 缺料／停產 | fail-safe 論證退為複合式 |
| **退路 3** | 同退路 2，但把 **DC+ 提高到 12V**（或極端情況下 24V，可直接併到 JD+ 那條 24V 軌） | 退路 2 實測 LED 電流仍不足以穩定吸合 | 🔴 **必須先由 A-2 步驟⑥ 的實測推算出板上限流電阻值**，再回算提高後的 I_F，確認**不超過光耦 LED 的絕對最大順向電流**（PC817 類典型連續 50mA）；且 2N2222A 的 V_CE 要承受 DC+ 全電壓（24V 仍在 V_CEO 40V 內 ✅）。副作用：觸發端電壓域被推回高壓，「觸發端與 ESP32 同軌」的好處消失 |
...
🔴 **偏好序原則不因分離供電而改變**：**裸繼電器方案永遠優先於任何 fallback**。
退路 2/3 只是把「裸繼電器也拿不到」時的可行空間寫清楚，不是把它們升格。

**驗收紀錄**：四項結果（含實測數值：吸合時 **IN 端電壓與灌入電流**、**5V 軌與 24V 軌實測電壓**、
**DC+ 側與 JD+ 側各自的耗流**）記在 §13 的 V-1/V-2 欄位，不要只打勾——
A-2 的實測餘裕是日後排錯的基準線，IN 端電流更是收斂 §3.4.4 兩個推定的唯一證據。
...
| WD-23 | LM2596 #2 OUT+ (24V) | K1 模組 **JD+** | 22 AWG | 🔄 **線圈側**供電（線圈 + 板載驅動級），~40mA。**分離供電模式：接 JD+，不是 DC+** |
| WD-24 | K1 模組 **JD−** | ⊖ 端子台 | 22 AWG | 🔄 線圈側回線（dock 星點） |
| WD-25 | *(已移除)* | — | — | 原為「Q1 射極 → ⊖ 端子台」。自焊驅動電路已由模組取代，本線號**作廢不使用**；線號不重編以免與既有標籤/照片對不上 |
| **WD-33** | LM2596 #1 OUT+ **(5.0V)** | K1 模組 **DC+** | 22 AWG | 🆕 **觸發側**供電（分離供電新增），~10–20mA。與 ESP32 VIN 同軌（§3.4.3、§3.6） |
| **WD-34** | ESP32 **GND** | K1 模組 **DC−** | 22 AWG | 🆕 觸發側回線（分離供電新增）。ESP32 GND 本身即在 ⊖ 星點上（§3.7） |
| — | K1 模組 **`DC+/JD+`、`DC−/JD−` 短接跳線帽 ×2** | **拆除** | — | 🔴 **施工檢查項，不是接線**：兩顆跳線帽必須拔除並裝袋貼標（A-4）。🔴 **跳線帽在位時嚴禁接 WD-23/WD-33 並上電——會把 24V 與 5V 兩軌直接短路**（§3.4.2） |
| — | K1 模組 **`H/L` 跳線** | **`H` 位** | — | 🔴 施工檢查項（A-4）。設成 `L` 會讓 §8 的 fail-safe 方向反轉 |

> **新增線號說明**：分離供電使模組低壓側從 3 條變成 5 條，新增的兩條取**現有最大線號之後**的
> **WD-33 / WD-34**（§4.3 用到 WD-32），**不重編既有線號**，以免與已印製的標籤／照片對不上。
> 模組低壓側完整清單：**WD-22（IN）／WD-23（JD+）／WD-24（JD−）／WD-33（DC+）／WD-34（DC−）**，
> 加上功率側 WD-5（COM）／WD-6（NO），NC 空置。
...
| — | *(交叉引用)* LM2596 #1 OUT+ / ESP32 GND 同時供 K1 模組觸發側 | 見 **WD-33 / WD-34**（§4.2） | — |
...
| **25** | RELAY_DRV | OUT | **直接接 K1 模組 IN**（光耦輸入，模組內建限流電阻；模組觸發側由 **5V 軌**經 DC+ 獨立供電，見 §3.4.3） | HIGH = K1 閉合（`H/L` 跳線在 **`H`**，A-4）。觸發電流 2–4mA、門檻 2.5–5V〔5V 版數據，待 A-2 確認〕 |
...
| 層 | 機制 | 為什麼與韌體無關 |
|---|---|---|
| **1. 接點使用方式** | 本體為 SLA-24VDC-SL-**C**（SPDT），但**只接 COM + NO，NC 端子空置禁用**（§3.3）。線圈失電 ⇒ COM–NO **開路** ⇒ 刷板無電 | 這是繼電器的**機械結構** + 一條可目視稽核的施工紀律（端子台上有絲印、NC 端貼禁用標籤）。裁決 2 要求的「常開接點」由此落實 |
| **2. 線圈電流的唯一來源** | 線圈電流只能經模組**板載輸出級**流過，而輸出級的導通完全取決於**光耦電晶體**是否被 LED 點亮 | 沒有第二條路徑能讓線圈通電。**分離供電下這一層更乾淨**：線圈電流全部來自 **JD+（24V）**，而 JD+ 只供電、不決定通斷；觸發側的 DC+（5V）根本不在線圈迴路上 |
...
這在**本設計選定的模式——分離供電（DC/JD 兩顆短接跳線帽已拆除）＋ `H/L` 跳線在 `H`（高電平觸發）——
...
> **A-1（§3.10）：分離供電（JD+ 24V／DC+ 5V）＋ `H` 位，IN 懸空、模組上電 10s，
> K1 必須不吸合（COM–NO 開路）。**
> A-1 不過 ⇒ 不得安裝，退回 §15 決策記錄 #3 的裸繼電器方案。

> **短接模式為什麼被禁用（釐清，不要記錯理由）**：
> 短接模式（跳線帽在位）**不會**反轉 fail-safe 方向——`H` 位下 IN 懸空一樣沒有 LED 電流。
> 禁用它的理由是**驅動可行性**（觸發端變 24V ⇒ 3.3V 直推灌不進 2–4mA ⇒ A-2 必不過，§3.4.2），
> 以及**接線安全**（JD+ 已接 24V、DC+ 已接 5V 時，跳線帽會短路兩條電源軌）。
> 把這兩件事混成「短接模式會破壞 fail-safe」是誇大，本文件不這樣寫。
...
> 亦即**同時**依賴自焊的 R2 與模組在 `L` 模式下的懸空行為（分離供電維持不變，DC+ 仍為 5V）。
...
| 當掉的形式 | 結果 | 依據 |
|---|---|---|
| ESP32 斷電 | GPIO 全部無驅動 ⇒ IN 無高電平 ⇒ 光耦 LED 無電流 ⇒ K1 開 | 純硬體 ✅（前提 A-1：分離供電 + `H` 位） |
| ESP32 硬體 reset（EN 腳、brown-out） | 同上，全程由光耦結構保證（含 bootloader 期間） | 純硬體 ✅（前提 A-1：分離供電 + `H` 位） |
| 韌體 panic / 例外 | ESP-IDF 預設 panic handler 行為為 **reset** ⇒ 同上 | 晶片機制 ✅ |
| **5V 軌失效**（LM2596 #1 掛掉／DC+ 線脫落） | 觸發側完全無供電 ⇒ 光耦 LED 必無電流 ⇒ K1 開。**ESP32 同時失電**（同一軌），不會出現「ESP32 活著但觸發側死掉」的半死狀態 | 分離供電新增的失效模式，方向安全 ✅ |
| **24V 軌失效**（LM2596 #2 掛掉／JD+ 線脫落） | 線圈直接失電 ⇒ K1 開。ESP32 仍活著並照常回報（5V 軌獨立） | 分離供電新增的失效模式，方向安全 ✅ |
...
| **D3** | **主繼電器 K1（模組）** | icshop「**30A 5V/12V/24V 繼電器模組-帶光耦**」的 **24V 版**<br/>🔴 下單時必須選 **24V** 線圈版（不是 5V/12V）<br/>連結：<https://www.icshop.com.tw/products/368030501112><br/>板上繼電器為 Songle **SLA-24VDC-SL-C**，本體印字 **30A 250VAC / 30VDC**<br/>🔴 **-C 為 SPDT，本設計只接 COM+NO，NC 空置貼「禁用」標籤**（§3.3）<br/>🔴 **採分離供電模式**：拆除 `DC+/JD+`、`DC−/JD−` 兩顆短接跳線帽，JD± 吃 24V、DC± 吃 5V，`H/L` 跳線設 `H`（§3.4）<br/>🔴 **到貨後必須先過 A-1 ~ A-4 才可安裝**（§3.10） | **2**（1 用 1 備） | icshop（現庫存 6 件） | **NT$79/片** |
...
| **D9** | DC-DC #1（5V） | **LM2596 模組**，輸入 4.5–40V、輸出調至 **5.0V**（供 ESP32 VIN **＋ K1 模組 DC+ 觸發側**，§3.4.3）。<br/>🔴 **必須是輸入輸出共地款**（收貨量 IN− ↔ OUT− ≈ 0Ω）。<br/>❌ **禁用 mini560**（輸入僅到 24V，28V 會燒）。<br/>🔥 建議加小散熱片 | 1 | 露天/蝦皮 | NT$30–50 |
| **D10** | DC-DC #2（24V） | **LM2596 模組**，輸出調至 **24.0V**（給 K1 模組 **JD+** 線圈側）。共地款同上。<br/>🔴 **必須與 D9 分開兩顆**，理由見 §3.6 | 1 | 同上 | NT$30–50 |
...
| D23 | 線材 **22 AWG 雙絞** | — | 微動開關線（1–2m ×2）、buck 供電、**K1 模組低壓側 4 條（JD± 24V／DC± 5V，WD-23/24/33/34）** | 光華商場、露天/蝦皮 | NT$100 |
...
| D27 | 排針 + 杜邦線 | 2.54mm，**母-母杜邦線 ≥6 條**（K1 模組低壓側 5 條 + 備用） | I2C、TP-PROBE 擴充點、**K1 模組 IN／JD±／DC±**（WD-22/23/24/33/34） | 光華商場、露天/蝦皮 | NT$50 |
| D28 | 束帶、標籤、熱縮套管 | — | 🔴 K1 模組端子線材固定、**NC 端子「禁用 — 空置」永久標籤**（§3.3）、線號標籤 | 五金行、露天/蝦皮 | NT$100 |
| **D29** | **小夾鏈袋 + 標籤**（跳線帽處置） | 2×3cm 以上夾鏈袋 ×2 | 🆕 收納 A-4 拆下的 **`DC+/JD+`、`DC−/JD−` 兩顆短接跳線帽**，貼標「**K1 模組 DC/JD 短接帽 — 分離供電下不得裝回**」，與模組一同保管（退貨或退回短接模式時要還原）。**不是消耗品，但必須列出，否則跳線帽會被隨手丟掉或誤插回**（§3.4.2） | 2 | 文具行、五金行 | NT$20 |
...
| 電控元件（D3–D18、D20），其中 **K1 模組 ×2 = NT$158（定價）** | ~NT$980–1,500 |
| 外殼（D19） | NT$300–600 |
| 線材與端子（D21–**D29**） | ~NT$650–870 |
| **合計** | **~NT$5,630–7,870** |
...
| # | 項目 | 何時驗 | 阻擋什麼 |
|---|---|---|---|
| **V-1** | 🔴 **K1 模組到貨驗收 A-1 ~ A-4 全數通過**（完整步驟與 fallback 見 **§3.10**，**全部在分離供電模式下執行**）：<br/>　A-1 IN 懸空不吸合／A-2 3.3V 直推可靠吸合／A-3 本體印字 SLA-**24**VDC-SL-C 且有 30A/30VDC 欄位／A-4 **兩顆 DC/JD 短接跳線帽已拆除且 `H/L` 在 `H` 位** | 收貨時，**安裝與接功率線之前** | **A-1 阻擋 §8 的整條 fail-safe 論證**（不過即不成立）；A-2 阻擋 §3.4 的直推接法（不過**優先退回裸繼電器方案**，2N2222A fallback 僅在裸繼電器缺料時採用）；A-3 阻擋 §3.3 的 DC 額定論證與裁決 2；A-4 阻擋 §8.1 的觸發方向，**且未拆跳線帽就接線會短路 5V 與 24V 兩軌**（§3.4.2） |
| **V-2** | 🔴 **觸發端實測（收斂 §3.4.4 的兩個推定）**：<br/>　① 吸合時 **IN 端灌入電流**（是否落在 2–4mA）與 IN 端對 DC− 電壓<br/>　② 由 ①推算板上 **IN 端限流電阻值** ⇒ 判定「24V 版觸發端是否等同 5V 版規格」<br/>　③ **DC+ 側**（5V 軌）與 **JD+ 側**（24V 軌）各自的實測耗流 | 收貨時（併入 A-2 步驟⑥執行） | ①② **阻擋 §3.4.4 的規格推定**——目前引用的 2–4mA / 2.5–5V 是**商品頁 5V 版數據**，24V 版是否適用只有實測能回答；③ 驗證 §3.6 的 5V 軌 ~10–20mA 與 24V 軌 ~40mA 估值（餘裕充足，僅供記錄與日後排錯基準線） |
...
| **R-10** | 🔴 **3.3V 直推的門檻裕度只有 0.8V**（§3.4.4：GPIO 3.3V vs 高電平門檻下限 2.5V，扣掉 GPIO 壓降後剩 ~0.6–0.7V） | 可能出現「常溫可吸合、低溫或元件離散下漏吸」的邊緣行為 | **A-2 含 20 次切換、24V 側降到 22V、5V 側降到 4.75V 三重壓力測試**，就是為了逼出邊緣行為；不過則**優先退回裸繼電器方案**（§15 #3），2N2222A 緩衝 fallback 僅在裸繼電器缺料時採用 |
| **R-11** | 🔴 **§3.4.4 引用的觸發端規格（2–4mA／2.5–5V）是商品頁的 5V 版數據**，「24V 版在分離供電下等同 5V 版」目前是**推定**，非實測 | 若 24V 版的 IN 端限流電阻是按 24V 選的，DC+ 改接 5V 也救不了 3.3V 直推 ⇒ A-2 不過 ⇒ 走退路 1 | **A-2 步驟⑥ + V-2** 實測 IN 端電流並回推電阻值即可收斂。文件已在 §3.3、§3.4.4、§5 三處標注此數據的來源與推定狀態，不得當成 24V 版的既成事實引用 |
...
| **13** | 🔄 **K1 改用 icshop 光耦繼電器模組（24V 版 ×2，NT$79/片），取代決策 3 的裸繼電器 + 自焊驅動** | 見 §15.1 詳述 | 決策 3 的裸繼電器方案（**降級為 fallback，未刪除**） |
| **14** | 🔧 **模組改採分離供電模式**：拆除 `DC+/JD+`、`DC−/JD−` 兩顆短接跳線帽，**JD± 吃 24V（線圈側）、DC± 吃 5V（觸發側，與 ESP32 同軌）**，`H/L` 跳線設 `H` | 見 §15.2 詳述（含**原理解錯在哪**的誠實記錄） | 短接模式（觸發端＝24V ⇒ 3.3V 直推必不過）；改用 5V 版模組（線圈就吃不到 24V 軌，且 24V 版採購已定案） |
